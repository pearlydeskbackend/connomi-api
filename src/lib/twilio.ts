...
export function smsConfirmation(name: string, service: string, date: string, time: string, clinicName: string, clinicPhone: string): string {
  return `Hi ${name}, your ${service} at ${clinicName} is confirmed for ${date} at ${time}. Reply CANCEL to cancel or call ${clinicPhone}.`
}

export function smsCancellation(name: string, service: string, date: string, time: string, clinicName: string, clinicPhone: string): string {
  return `Hi ${name}, your ${service} on ${date} at ${time} at ${clinicName} has been cancelled. Call ${clinicPhone} to rebook anytime.`
}

export function smsReschedule(name: string, service: string, date: string, time: string, clinicName: string, clinicPhone: string): string {
  return `Hi ${name}, your ${service} at ${clinicName} has been rescheduled to ${date} at ${time}. Questions? Call ${clinicPhone}.`
}

export function smsReminder(name: string, service: string, date: string, time: string, clinicName: string, clinicPhone: string): string {
  return `Hi ${name}, reminder: you have a ${service} at ${clinicName} tomorrow ${date} at ${time}. Reply CONFIRM or call ${clinicPhone}.`
}

export function smsRecall(name: string, clinicName: string, clinicPhone: string): string {
  return `Hi ${name}, it has been 6 months since your last cleaning at ${clinicName}. Call ${clinicPhone} to book or reply YES.`
}

export function smsReview(name: string, clinicName: string, reviewLink: string): string {
...
export function smsUrgentMessage(patientName: string, phone: string, message: string, urgency: string): string {
...
export function smsSmsReply(clinicName: string, clinicPhone: string): string {
